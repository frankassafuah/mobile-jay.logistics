...
$(document).ready(function(){      
	...
	$( "#booking_previous_2" ).on( "click", function() {
		$("#book_1").show(); 
		$("#book_2").slideUp();				
	})
	...
	$( "#register_button" ).on( "click", function() {
       ...
       var register_user_type = $('#register_user_type').val();
       var register_email = $('#register_email').val();
	   var device_platform = $('#device_platform').val();
	   var device_id = $('#device_id').val();
	   register(register_first_name,register_last_name,register_number,register_password,register_user_type,register_email,device_platform,device_id);
	})
		...
		if (forgot_phone) {
			$('#AjaxLoader').show();
				$.ajax({
				  ...
				  data: {forgot_phone:forgot_phone},
					...
					console.log(res);
						...
						if(res.stat == 0){ 
							swal('Account could not be found')
						} 
				...
				swal('Could not verify your number');
				$('#forgot_phone').val('');
				$('#AjaxLoader').hide();
			}	
		}
			...
			$.ajax({
			  ...
			  data: {forgot_phone:forgot_phone,
				...
				console.log(res);
					...
					if(res.updated == 0 ){
						swal('There was an issue, please try again.');
	...
	function register(register_first_name,register_last_name,register_number,register_password,register_user_type, register_email, device_platform,device_id){
		if (register_number && register_password) {
			...
			swal('An account with your details already exist.');
			$('#register_first_name').val('');
			$('#register_last_name').val('');
			$('#register_number').val('');
			$('#register_password').val('');
			...
			$.ajax({
			  ...
			  data: { number: login_number, password:login_password},
				...
				console.log(res);
					...
					if(res.stat == 0 ){
						purgeData();
						swal('Your details were incorrect, please try again.');
						$('#login_number').val('');
						$('#login_password').val('');
					}
					$('#AjaxLoader').hide();
