...
$(document).ready(function(){      
	...
	var login_url = ajax_base + "manager/Vlogin";
	var register_url = ajax_base + "manager/Vregister";
	var forgot_url = ajax_base + "manager/Vforgot";
	var forgot_verify_url = ajax_base + "manager/VforgotVerify";
	var forgot_action_url = ajax_base + "manager/VforgotAction";
	var profile_url = ajax_base + "manager/profile";
	var orders_url = ajax_base + "manager/user_orders";
	...
	// PURGE 
	function purgeData(){
		window.localStorage.removeItem('JayLogisticGen');
		window.localStorage.removeItem('JayLogisticStat');
	}
	
	// PROFILE EDIT DETIALS 
	$( "#profile_edit_button" ).on( "click", function() {
		$("#profile_edit_div").show(); 
		$("#profile_main_div_buttons").hide();		
		$("#profile_main_div").hide();
		
	})
	
	$( "#profile_edit_cancel_button" ).on( "click", function() {
		$("#profile_edit_div").hide();
		$("#profile_main_div_buttons").show();				
	...
	$( "#profile_edit_save_button" ).on( "click", function() {
		...
		var profile_edit_number = $('#profile_number').html();
		
	    profileEdit(profile_edit_first_name,profile_edit_last_name,profile_edit_number);
	...
	$( "#login_button" ).on( "click", function() {
	   ...
	   login(login_number,login_password);
	})
	
	// START RECOVERY PROCESS
	$( "#recover_verification_button" ).on( "click", function() {
	   var forgot_phone = $('#forgot_phone').val();
	   forgot(forgot_phone);
	})
	
	// VERIFY WITH CODE
	$( "#recover_verify_button" ).on( "click", function() {
	   var forgot_code = $('#forgot_code').val();
	   var gen_code = $('#code').val();
	   forgotVerify(forgot_code,gen_code);
	})
	
	// COMPLETE RECOVER ACTION
	$( "#recover_action_button" ).on( "click", function() {
	   var forgot_phone = $('#forgot_phone').val();
	   var forgot_code = $('#forgot_code').val();
	   var forgot_password = $('#forgot_password').val();
	   forgotAction(forgot_phone,forgot_password);
	})
	...
	$( "#register_button" ).on( "click", function() {
	   ...
	   var device_platform = $('#device_platform').val();
	   var device_id = $('#device_id').val();
	   register(register_first_name,register_last_name,register_number,register_password,device_platform,device_id);
	})
	
	
	
	function profileEdit(profile_edit_first_name,profile_edit_last_name,profile_edit_number){
		if (profile_edit_number) {
			$('#AjaxLoader').show();
				$.ajax({
				  url: profile_edit_url,
				  method: "POST",
				  data: {profile_edit_first_name:profile_edit_first_name,
						 profile_edit_last_name:profile_edit_last_name,
						 profile_edit_number:profile_edit_number},
				  dataType: 'json',
				}).done(function(res) {
					console.log(res);
						if(res.stat == 1){ 
		...
		if (forgot_phone) {
			$('#AjaxLoader').show();
				$.ajax({
				  ...
				  method: "POST",
				  dataType: 'json',
				  data: {forgot_phone:forgot_phone},
				}).done(function(res) {
					console.log(res);
						if(res.stat == 1){ 
							$('#code').val(res.v_code);
							// HIDE VERIFICATION BUTTON
							$("#recover_verification_button").css("display", "none");
							// SHOW VERIFY BUTTON
							$("#recover_verify_button").css("display", "block");
							$("#forgot_verification_div").css("display", "block");					
						} 
				...
				alert('Could not verify your number');
				$('#forgot_phone').val('');
				$('#AjaxLoader').hide();
			}	
		}
		
		function forgotVerify(forgot_code,gen_code){
		if (forgot_code && gen_code) {
			$('#AjaxLoader').show();
				$.ajax({
				  url: forgot_verify_url,
				  method: "POST",
				  dataType: 'json',
				  data: {forgot_code:forgot_code, gen_code:gen_code},
				}).done(function(res) {
					console.log(res);
						if(res.code == 100){ 
							alert("Verified");
							//HIDE OLD BUTTONS
							$("#recover_verification_button").css("display", "none");
							$("#recover_verify_button").css("display", "none");
							
							//SHOW NEW PASSWORD Box
							$("#forgot_password_div").css("visibility", "visible");
							
							// show
							$("#recover_action_button").css("display", "block");
						...
						if(res.code == 200){ 
							...
							$('#forgot_phone').val('');
							$('#forgot_code').val('');
							$("#forgot_verification_div").css("display", "none");
							
							//RECOVER ARE BUTTONS
							$("#recover_verification_button").css("display", "block");
							$("#recover_verify_button").css("display", "none");
							$("#recover_action_button").css("display", "none");
						} 
						$('#AjaxLoader').hide();
				});
	...
	function forgotAction(forgot_phone,forgot_password){
		if (forgot_phone && forgot_password) {
			$('#AjaxLoader').show();
			$.ajax({
			  url: forgot_action_url,
			  method: "POST",
			  dataType: 'json',
			  data: {forgot_phone:forgot_phone,
				...
				console.log(res);
					...
					if(res.updated == 0 ){
						...
						window.location.href = "page-forgot-1.html";
					}
				$('#AjaxLoader').hide();
			});
	...
	function register(register_first_name,register_last_name,register_number,register_password,device_platform,device_id){
		if (register_number && register_password) {
			$('#AjaxLoader').show();
			$.ajax({
			  url: register_url,
			  method: "POST",
			  dataType: 'json',
			  data: {register_first_name:register_first_name,
			...
			alert('An account with your details already exist.');
			$('#register_first_name').val('');
			$('#register_last_name').val('');
			$('#register_number').val('');
			$('#register_password').val('');
	...
	function login(login_number,login_password){
		if (login_number && login_password) {
			$('#AjaxLoader').show();
			$.ajax({
			  url: login_url,
			  method: "POST",
			  dataType: 'json',
			  data: { number: login_number, password:login_password},
			}).done(function(res) {
				console.log(res);
					//IF LOGIN SUCCESSFUL
					if(res.stat == 1){ 
						window.localStorage.setItem('JayLogisticGen', login_number);
						window.localStorage.setItem('JayLogisticStat', 1);
